...
public class ScaleRoutineStartOnRight implements AutonomousRoutine {
	...
	public ArrayList<AutonomousCommand> getAutonomousCommands() {
		...
		//if scale is on our side, go for SCALE -- RRR, LRL -- then hunting
		if(PlateAssignmentReader.GetScaleSide() == 'R') 
		{
			...
			returnValue.add(new TurnRobotToAngle(mDriveTrain, 270)); //turn to face scale
		...
		//if scale is not on our side but switch is, go for SWITCH -- RLR -- then hunting
		else if(PlateAssignmentReader.GetNearSwitchSide() == 'R') 
		{
			returnValue.add(new TurnWheelsToAngle(mDriveTrain, 0)); //make sure wheels are forward
			...
			returnValue.add(new TurnRobotToAngle(mDriveTrain, 270)); //turn to face switch
		...
		//if neither scale nor switch are on our side, just go forward -- LLL -- then on_way_to_exchange? how do diff :O
		else {
